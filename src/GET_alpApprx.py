...
def initialize_parameters(choice, net, warm_starts, Y_train, device):

    a, b, c = None, None, None
    if choice < len(warm_starts) and warm_starts[choice] is not None:
        # print("warm start")
        ws = warm_starts[choice]
...
def treeOptbyGRADwithC(treeDepth, indices_flags_dict, epochNum, X_train, Y_train, device, warm_starts, scaleFactor, lrscheList,  idx_start, callback):
    ...
    a, b, c = initialize_parameters(idx_start, net, warm_starts, Y_train, device)
...
def multiStartTreeOptbyGRAD_withC(X_train, Y_train, treeDepth, indices_flags_dict, epochNum, device, warm_starts, startNum):
    ...
    if len(warm_starts) > startNum:
        startNum = len(warm_starts)
    for idx_start in range(startNum):
        ...
        ObjvAlp = 1e10
        bestTreeAlp = None
        warm_starts_cur = [warm_starts[idx_start]] if idx_start < len(warm_starts) else  [None]
        idxCurr = 0
        
        for scaleFactor in scaleList:
            objvCurr, r2Curr, treeCurrent = treeOptbyGRADwithC(treeDepth, indices_flags_dict, epochNum, X_train, Y_train, device, warm_starts_cur, scaleFactor, lrscheList, idxCurr, callback)
            if objvCurr < ObjvAlp:
                ObjvAlp = objvCurr
                bestTreeAlp = copy.deepcopy(treeCurrent)
            warm_starts_cur.append( {"a": treeCurrent["a"], "b": treeCurrent["b"]* (-1.0), "c": treeCurrent["c"]})
            idxCurr += 1
